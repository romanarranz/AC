/* 
	...
	Para compilar usar (-lrt: real time library):	gcc -fopenmp -O2 SumaVectoresCParallelSections.c -o SumaVectoresCParallelSections -lrt
	Para ejecutar use: SumaVectoresC longitud

	Para obtener el ensamblador compilar con : gcc -O2 SumaVectoresCParallelSections.c -S -o SumaVectoresCParallelSections.s -lrt
*/
...
int main(int argc, char** argv){
	...
	#pragma omp parallel sections private(i) // con private i nos deja una copia de i en cada section, para hacer uso de i de forma local
	{
		#pragma omp section
		{
			for(i=0; i<N; i++)
	...
	#pragma omp parallel sections private(i)
	{
		#pragma omp section
		{
			//Calcular suma de vectores desde el princpio hasta la mitad
			for(i=0; i<N/2; i++){
				v3[i] = v1[i] + v2[i];
			}
		}
		#pragma omp section
		{
			//Calcular suma de vectores desde la mitad hasta el final
			for(i=N/2; i<N; i++){
				v3[i] = v1[i] + v2[i];
			}
		}
